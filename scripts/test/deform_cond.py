...
def process(index, all_mesh, all_canonical_mask, points_sample, save_dir):                
        ...
        # ax.scatter(to_cpu(source_pt)[:, 0], to_cpu(source_pt)[:, 1], to_cpu(source_pt)[:, 2], c='b', marker='o')
        ...
        source.export(os.path.join(save_dir, mask_name + '_canonical.obj'))
    ...
    if method == 'l-w ratio':
        ...
        for i in all_files:
            if i.endswith('.obj') and not 'maple' in i:
                ...
                mask_five = np.argsort(np.abs(np.array(mask_ratios) - ratio))[:4]
                # with Pool(4) as pool:
                #     for index in mask_five:
                #         pool.apply_async(process, args=(index, all_mesh, all_canonical_mask, points_sample, save_dir))
                #     pool.close()
                #     pool.join()
                for index in mask_five:
